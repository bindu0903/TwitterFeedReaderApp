...
namespace Utils.Twitter
{
    public class TwitterHelper
    {
        ...
        public IEnumerable<Tweet> GetSearchTweets(string q, long? sinceId = null, long? maxId = null, int? count = 10, string screenName = "")
        {
            return GetSearchTimeline("https://api.twitter.com/1.1/search/tweets.json",q, sinceId, maxId, count, screenName );
        }

        private IEnumerable<Tweet> GetSearchTimeline(string url,string q, long? sinceId, long? maxId, int? count, string screenName)
        {
            ...
            return tweets.Cast<Dictionary<string, object>>().Select(tweet =>
            {
                ...
                return new Tweet
                {
                    ...
                    Urls = IsPropertyExist(entities, "urls") && entities["urls"] != null ? (object[])entities["urls"]  : new object[0] ,
        ...
        private IEnumerable<Tweet> GetTimeline(string url, long? sinceId, long? maxId, int? count, string screenName)
        {
            ...
            return tweets.Cast<Dictionary<string, object>>().Select(tweet =>
            {
                ...
                return new Tweet
                {
                    ...
                    ProfileImageUrl = (string)user["profile_image_url"],
                    ProfileBannerUrl = (string)user["profile_banner_url"],
                    Urls = entities.ContainsKey("urls") && entities["urls"] != null ? (object[])entities["urls"] : new object[0],
                    Media = entities.ContainsKey("media") && entities["media"] != null ? (object[])entities["media"] : new object[0],
                };
            }).ToArray();
        }
